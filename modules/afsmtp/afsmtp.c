...
afsmtp_dd_init(LogPipe *s)
{
  ...
  if (!log_threaded_dest_driver_init_method(&self->super,
                                            ...
                                            SCS_SMTP, afsmtp_dd_format_stats_instance(self)))
    return FALSE;
...
static void
afsmtp_dd_disconnect(LogThrDestDriver *d)
{
}
...
afsmtp_dd_new(void)
{
  ...
  self->super.worker.init = afsmtp_worker_thread_init;
  self->super.worker.deinit = afsmtp_worker_thread_deinit;
  self->super.worker.disconnect = afsmtp_dd_disconnect;
