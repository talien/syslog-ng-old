...
log_threaded_dest_driver_worker_thread_main(gpointer arg)
{
  ...
  if (self->worker.deinit)
    self->worker.deinit(self);
...
gboolean
log_threaded_dest_driver_init_method(LogThrDestDriver *self,
                                     gchar *persist_name,
                                     gint stats_source, gchar *stats_instance)
{
  GlobalConfig *cfg = log_pipe_get_config((LogPipe *)self);

  if (!log_dest_driver_init_method((LogPipe *)self))
    return FALSE;
  ...
  stats_lock();
  stats_register_counter(0, stats_source | SCS_DESTINATION, self->super.super.id,
                         stats_instance,
...
gboolean
log_threaded_dest_driver_deinit_method(LogThrDestDriver *self,
                                       gint stats_source, gchar *stats_instance)
{
  ...
  stats_lock();
  stats_unregister_counter(stats_source | SCS_DESTINATION, self->super.super.id,
                           stats_instance,
                           SC_TYPE_STORED, &self->stored_messages);
  stats_unregister_counter(stats_source | SCS_DESTINATION, self->super.super.id,
                           stats_instance,
  ...
  if (!log_dest_driver_deinit_method((LogPipe *)self))
    return FALSE;
...
log_threaded_dest_driver_init_instance(LogThrDestDriver *self)
{
  ...
  self->super.super.super.init = NULL;
  self->super.super.super.deinit = NULL;
