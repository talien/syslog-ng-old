...
struct _LogThrDestDriver
{
  ...
  struct
  {
    void (*init) (LogThrDestDriver *s);
    gboolean (*insert) (LogThrDestDriver *s);
    void (*disconnect) (LogThrDestDriver *s);
    void (*deinit) (LogThrDestDriver *s);
  } worker;

  void (*queue_method) (LogThrDestDriver *s);
};

gboolean log_threaded_dest_driver_init_method(LogThrDestDriver *self,
                                              gchar *persist_name,
                                              gint stats_source,
                                              gchar *stats_instance);
gboolean log_threaded_dest_driver_deinit_method(LogThrDestDriver *self,
                                                gint stats_source,
                                                gchar *stats_instance);
