...
GString* rss_format_backlog(RssDestDriver* self, GString* result)
{
    GList* tmp = self->backlog;
    GString* message;
    char id_str[10];
    int offset = self->id;
    result = g_string_append(result, "<?xml version=\"1.0\"?>\n<feed xmlns=\"http://www.w3.org/2005/Atom\">\n<title>syslog-ng</title><link>localhost:8080</link>");
    while (tmp)
    {
        message = g_string_new("");
        log_template_format(self->template, tmp->data, NULL, LTZ_LOCAL, 0, NULL, message);
        msg_debug("Formatted log:", evt_tag_str("msg", message->str), NULL);
...
static int init_rss_listen_socket(RssDestDriver* self)
{
...
static int accept_rss_connection(int sock)
{
   struct sockaddr_in rsockaddr;
   int addrlen;
   msg_debug("Connection accepted", NULL);
   return accept(sock, &rsockaddr, &addrlen);
}

static GString* create_http_header()
{
  return g_string_new("HTTP/1.1 200 OK\n Content-Type:application/atom+xml\n\n");
}

static void respond_with_feed(int sock, GString* feed)
{
  gchar buffer[4096];
  msg_debug("Feed response", evt_tag_str("response",feed->str),NULL);
  read(sock, buffer, sizeof(buffer));
  write(sock, feed->str, feed->len); 
}

static void accept_and_serve_rss_connection(RssDestDriver* self)
{
    GString* feed;
    int newsock, sock;
...
static void rss_append_msg_to_backlog(RssDestDriver* self, LogMessage* msg)
{
...
static void
rss_dd_queue(LogPipe *s, LogMessage *msg, const LogPathOptions *path_options, gpointer user_data)
{
  RssDestDriver *self = (RssDestDriver *) s;
  msg_notice("Rss plugin received a message",
...
void rss_dd_set_port(LogDriver* s, int port)
{
    RssDestDriver* self = (RssDestDriver*) s;
    self->port = port;
}

gboolean rss_dd_init(LogPipe* s)
{
  GError* error = NULL;
  RssDestDriver* self = (RssDestDriver*) s;
  int listen_socket;
...
gboolean rss_dd_deinit(LogPipe* s)
{
  RssDestDriver* self = (RssDestDriver*) s;
  //iv_fd_unregister(&self->listen_fd);
}

void rss_dd_free(LogPipe* s)
{
  RssDestDriver* self = (RssDestDriver*) s;
...
const ModuleInfo module_info =
{
  .canonical_name = "rss",
  .version = VERSION,
  .description = "The rss module is a sample destination driver not really doing any kind of useful work.",
